...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        # default="dreambooth/model.pt", # load from local trained dreambooth
        default=None, # load from hugging face (cache)
    ...
    for j in range(3):
        ...
        Image.fromarray(img[0]).save(f"{args.text}-{j}.png")

    images = np.concatenate([np.concatenate(img, 1) for img in images_matrix], 0)
    Image.fromarray(images).save(f"{args.text}.png")
